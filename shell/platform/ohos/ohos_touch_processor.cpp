...
void OhosTouchProcessor::HandlePinchEvent(
  ...
  // 获取设备ID和事件时间
  int32_t deviceId = OH_ArkUI_UIInputEvent_GetDeviceId(event);
  ...
  if (touchEvent1 == nullptr) {
    LOGE("HandlePinchEvent: Failed to allocate touchEvent1");
    return;
  }
  auto touchEvent2 = std::make_unique<OH_NativeXComponent_TouchEvent>();
  if (touchEvent2 == nullptr) {
    LOGE("HandlePinchEvent: Failed to allocate touchEvent2");
    return;
  }
...
void OhosTouchProcessor::HandleFlingEvent(
  ...
  if (touchEvent == nullptr) {
    LOGE("HandleFlingEvent: Failed to allocate touchEvent");
    return;
  }
