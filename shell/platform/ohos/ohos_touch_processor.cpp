...
OhosTouchProcessor::OhosTouchProcessor()
  ...
  if (localLibHandler_ == nullptr) {
    ...
    // 当 API 版本大于等于 14 时加载 OH_ArkUI_UIInputEvent_GetDeviceId (API14)
    if ( apiVersion_ >= 14){
    ...
    // 当 API 版本大于等于 15 时加载 OH_ArkUI_AxisEvent_GetAxisAction (API15)
    if (apiVersion_ >= 15) {
    ...
    // 当 API 版本大于等于 17 时加载 OH_ArkUI_UIInputEvent_GetModifierKeyStates (API18)
    if (apiVersion_ >= 17) {
...
// 处理轴事件：触控板 捏合缩放 和 滚动抛滑 手势，鼠标 滚轮滑动 和 Ctrl+滚轮缩放
void OhosTouchProcessor::HandleAxisEvent(
  ...
  if (OH_GetSdkApiVersion() < 15) {
    FML_LOG(ERROR) << "HandleAxisEvent: API version is lower than 15, skipping axis event handling.";
    return;
  }
