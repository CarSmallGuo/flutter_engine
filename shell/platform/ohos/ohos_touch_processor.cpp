...
constexpr int KEYCODE_CTRL = 1 << 0;
constexpr int TOOL_TYPE_MOUSE = 3;
...
void OhosTouchProcessor::HandleAxisEvent(int64_t shell_holderID,
  ...
  if(toolType == TOOL_TYPE_MOUSE && (keyStates & KEYCODE_CTRL)) {
    // Ctrl + 鼠标滚轮
...
void OhosTouchProcessor::HandlePinchEvent(int64_t shell_holderID,
  ...
  // For DFX
  fml::closure task = [timeStampDFX = eventTime](void) {
    FML_TRACE_EVENT("flutter", "HandlePinchEventUI", "timeStamp", timeStampDFX);
  };
}
...
void OhosTouchProcessor::HandleFlingEvent(int64_t shell_holderID,
  ...
  // For DFX
  fml::closure task = [timeStampDFX = eventTime](void) {
    FML_TRACE_EVENT("flutter", "HandleFlingEventUI", "timeStamp", timeStampDFX);
  };
}
...
void OhosTouchProcessor::HandleScaleEvent(int64_t shell_holderID,
  ...
  // 获取事件时间
  int64_t eventTime = OH_ArkUI_UIInputEvent_GetEventTime(event);
  FML_TRACE_EVENT("flutter", "HandleScaleEvent", "timeStamp", eventTime);
  ...
  // For DFX
  fml::closure task = [timeStampDFX = eventTime](void) {
    FML_TRACE_EVENT("flutter", "HandleScaleEventUI", "timeStamp", timeStampDFX);
  };
}
