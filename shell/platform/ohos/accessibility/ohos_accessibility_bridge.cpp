 ...
 */
void OhosAccessibilityBridge::PageStateUpdate() 
{
    ...
    ArkUI_AccessibilityElementInfoList* elementList)
{
  ...
  //创建并配置节点id >= 1的全部节点
  for (int64_t i = elementId + 1; i < elementInfoCount; i++) {
    auto childNode = GetFlutterSemanticsNode(static_cast<int32_t>(i));
    //当节点为隐藏状态时，自动规避
