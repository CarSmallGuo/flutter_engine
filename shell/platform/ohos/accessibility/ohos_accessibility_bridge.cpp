...
void OhosAccessibilityBridge::FlutterNodeToElementInfoById(
  ...
  //获取当前节点的组件类型
  std::string componentTypeName = GetNodeComponentType(flutterNode);
  FML_DLOG(INFO) << "FlutterNodeToElementInfoById componentTypeName = " << componentTypeName;
...
int32_t OhosAccessibilityBridge::ExecuteAccessibilityAction(
  ...
  // char* value;
  // int32_t ret =
  // OH_ArkUI_FindAccessibilityActionArgumentByKey(actionArguments, key.c_str(),
  // &value); if(value == nullptr) {
  //   FML_DLOG(ERROR)<<"ExecuteAccessibilityAction
  //   OH_ArkUI_FindAccessibilityActionArgumentByKey value = null"; return
  //   ARKUI_ACCESSIBILITY_NATIVE_RESULT_FAILED;
  // }
  // if(ret ==
  // ArkUI_AcessbilityErrorCode::ARKUI_ACCESSIBILITY_NATIVE_RESULT_BAD_PARAMETER)
  // {
  //   FML_DLOG(ERROR)<<"ExecuteAccessibilityAction
  //   OH_ArkUI_FindAccessibilityActionArgumentByKey BAD_PARAMETER";
  // } else if(ret ==
  // ArkUI_AcessbilityErrorCode::ARKUI_ACCESSIBILITY_NATIVE_RESULT_FAILED) {
  //   FML_DLOG(ERROR)<<"ExecuteAccessibilityAction
  //   OH_ArkUI_FindAccessibilityActionArgumentByKey RESULT_FAILED";
  // } else {
  //   FML_DLOG(INFO)<<"ExecuteAccessibilityAction
  //   OH_ArkUI_FindAccessibilityActionArgumentByKey RESULT_SUCCESSFUL";
  // }

  // 根据当前elementid和无障碍动作类型，发送无障碍事件
  ArkUI_AccessibilityEventType eventType = ArkUI_AccessibilityEventType::
      ARKUI_ACCESSIBILITY_NATIVE_EVENT_TYPE_ACCESSIBILITY_FOCUSED;
  Flutter_SendAccessibilityAsyncEvent(elementId, eventType);

  FML_DLOG(INFO)
      << "OhosAccessibilityBridge::ExecuteAccessibilityAction is end";
  return ARKUI_ACCESSIBILITY_NATIVE_RESULT_SUCCESSFUL;
}
