...
void OhosVsyncVotingMgr::VoteAnimationValue(AnimationType ANType, double devicePixelRatio, double velocity)
{
    ...
    if (devicePixelRatio != 0.0) {
        // V毫米 = V像素 * 25.4 / (devicePixelRatio * 160)
...
inline void OhosVsyncVotingMgr::VoteANTranslate(double velocity)
{
    ...
    for (std::vector<std::map<string, int> >::iterator it = framesSet.begin(); it != framesSet.end(); it++) {
    ...
    if (!firstVoteFrame_ && (expectedRateTmp == PlatformViewOHOSNapi::display_refresh_rate)) {
        return;
    }
...
void OhosVsyncVotingMgr::VotingByNativeVsync(OH_NativeVSync* handle)
{
    ...
    if (resultFrameRate_ == PlatformViewOHOSNapi::display_refresh_rate && resultFrameRate_ == localFrameRate_) {
        return;
    }
...
void OhosVsyncVotingMgr::ParseFramesCfg(void)
{
    ...
    std::unique_ptr<fml::Mapping> framesCfgMapping = asset_provider_->GetAsMapping(std::string(FRAMES_CFG_JSON));
    if (framesCfgMapping == nullptr) {
        FML_LOG(ERROR) << "Failed to GetAsMapping";
        return;
    }

    const char* data = reinterpret_cast<const char*>(framesCfgMapping->GetMapping());
    if (data ==nullptr) {
...
void OhosVsyncVotingMgr::SetAssetProvider(std::unique_ptr<OHOSAssetProvider> hap_asset_provider)
{
