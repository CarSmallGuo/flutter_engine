...
namespace fml{
    ...
    namespace hiappevent{
        ...
        void OhosHiappEventDDL::init(void)
        {
            ...
            do {
                createProcessorFunc_ = reinterpret_cast<CreateProcessorFunc>(dlsym(libhiappevent_ndk_handler_, "OH_HiAppEvent_CreateProcessor"));
                if (createProcessorFunc_ == nullptr) {
                    FML_LOG(ERROR) << "createProcessorFunc_ error";
                    break;
                }

                setReportRouteFunc_ = reinterpret_cast<SetReportRouteFunc>(dlsym(libhiappevent_ndk_handler_, "OH_HiAppEvent_SetReportRoute"));
                if (setReportRouteFunc_ == nullptr) {
                    FML_LOG(ERROR) << "setReportRouteFunc_ error";
                    break;
                }

                setReportPoliceFunc_ = reinterpret_cast<SetReportPoliceFunc>(dlsym(libhiappevent_ndk_handler_, "OH_HiAppEvent_SetReportPolicy"));
                if (setReportPoliceFunc_ == nullptr) {
                    FML_LOG(ERROR) << "setReportPoliceFunc_ error";
                    break;
                }

                setReportEventFunc_ = reinterpret_cast<SetReportEventFunc>(dlsym(libhiappevent_ndk_handler_, "OH_HiAppEvent_SetReportEvent"));
                if (setReportEventFunc_ == nullptr) {
                    FML_LOG(ERROR) << "setReportEventFunc_ error";
                    break;
                }

                addFunc_ = reinterpret_cast<AddFunc>(dlsym(libhiappevent_ndk_handler_, "OH_HiAppEvent_AddProcessor"));
                if (addFunc_ == nullptr) {
                    FML_LOG(ERROR) << "addFunc_ error";
                    break;
                }

                addStringParam_ = reinterpret_cast<AddStringParam>(dlsym(libhiappevent_ndk_handler_, "OH_HiAppEvent_AddStringParam"));
                if (addStringParam_ == nullptr) {
                    FML_LOG(ERROR) << "addStringParam_ error";
                    break;
                }

                addInt32Param_ = reinterpret_cast<AddInt32Param>(dlsym(libhiappevent_ndk_handler_, "OH_HiAppEvent_AddInt32Param"));
                if (addInt32Param_ == nullptr) {
                    FML_LOG(ERROR) << "addInt32Param_ error";
                    break;
                }

                addInt64Param_ = reinterpret_cast<AddInt64Param>(dlsym(libhiappevent_ndk_handler_, "OH_HiAppEvent_AddInt64Param"));
                if (addInt64Param_ == nullptr) {
                    FML_LOG(ERROR) << "addInt64Param_ error";
                    break;
                }

                write_ = reinterpret_cast<Write>(dlsym(libhiappevent_ndk_handler_, "OH_HiAppEvent_Write"));
                if (write_ == nullptr) {
                    FML_LOG(ERROR) << "write_ error";
                    break;
                }

                destroyParamList_ = reinterpret_cast<DestroyParamList>(dlsym(libhiappevent_ndk_handler_, "OH_HiAppEvent_DestroyParamList"));
